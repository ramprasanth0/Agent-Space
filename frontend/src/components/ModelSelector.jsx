...
export default function ModelSelector({ models, selected, onSelect }) {
    ...
    return (
        <div className="bg-pomp_and_power-400 m-3 rounded-full flex gap-2 p-2 justify-center">
        {models.map((m) => (
            <button
                key={m}
                onClick={() => onSelect(m)}
                className={`flex max-w-xs px-4 py-2 rounded-full transition font-semibold border
                    // ${selected === m ? "active" : ""}
                    ${selected === m
                    ? "bg-english-violet-800 border-none text-night-100"
                    : "bg-english-violet border-none text-white hover:bg-english-violet-700"}
                    `}
                >
                {/* Image placed here */}
